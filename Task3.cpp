                ...
                double *cx0, double *cy0, double *cx1, double *cy1)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (p[i] == 0) {
            ...
            double r = q[i] / p[i];
            if (p[i] < 0) {
                if (r > u2) return 0; // Outside
                if (r > u1) u1 = r;   // Entering
            } else {
                if (r < u1) return 0; // Outside
                if (r < u2) u2 = r;   // Leaving
            }
        }
    }
...
{
    glColor3f(1.0f, 0.6f, 0.0f); // Orange rectangle
    glLineWidth(2.5f);
...
void display()
{
    ...
    // Draw clipped lines in cyan
    glColor3f(0.0f, 0.8f, 0.8f);
    glLineWidth(3.0f);
    glBegin(GL_LINES);
    for (int i = 0; i < nlines; i++) {
        double cx0, cy0, cx1, cy1;
        if (liangBarsky(lines[i].x0, lines[i].y0, lines[i].x1, lines[i].y1,
...
void initOpenGL(double xmin, double ymin, double xmax, double ymax)
{
    ...
    double margin = 20.0;
    double left   = xmin - margin;
    double right  = xmax + margin;
    double bottom = ymin - margin;
    double top    = ymax + margin;
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Liang-Barsky Line Clipping Demo");
